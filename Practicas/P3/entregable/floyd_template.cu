...
__global__ void floyd_kernel1D(int * M, const int nverts, const int k) {
  int ij = blockIdx.x * blockDim.x + threadIdx.x, // celda ij en la matriz
      i = ij / nverts,                            // índice i en la matriz
      j = ij - i * nverts;                        // índice j en la matriz
...
__global__ void floyd_kernel1DShared(int * d_M, const int nverts, const int k) {
  int blockPos = blockIdx.x * blockDim.x, // posición inicial del bloque en memoria
...
__global__ void floyd_kernel2DShared(int * d_M, const int nverts, const int k) {
  ...
  if (i < nverts && j < nverts) {
    ...
    l_i = threadIdx.y;  // índice i en la submatriz
    l_j = threadIdx.x;  // índice j en la submatriz

    if (l_i == 0) {                       // la primera fila
      s_filK[l_j] = d_M[k * nverts + j];  // copia en la fila k
    }
    if (l_j == 0) {                       // la primera columna
      s_colK[l_i] = d_M[i * nverts + k];  // copia en la columna k
    }
...
// Kernel Shared 2D V2
__global__ void floyd_kernel2DSharedV2(int * d_M, const int nverts, const int k) {
  int i = blockIdx.y * blockDim.y + threadIdx.y,  // índice i en la matriz
      j = blockIdx.x * blockDim.x + threadIdx.x,  // índice j en la matriz
      ij, l_i, l_j, l_ij;
  ...
  if (i < nverts && j < nverts) {
    __shared__ int s_filK[BLOCK_SIZE_2D],
                   s_colK[BLOCK_SIZE_2D],
                   s_M[BLOCK_SIZE_2D * BLOCK_SIZE_2D];

    l_i = threadIdx.y;                // índice i en la submatriz
    l_j = threadIdx.x;                // índice i en la submatriz
    l_ij = l_i * BLOCK_SIZE_2D + l_j; // celda ij en la submatriz
    ij = i * nverts + j;              // celda ij en la matriz

    s_M[l_ij] = d_M[ij];                  // todos copian en la submatriz compartida
    ...
    if(i != j && i != k && j != k) {
      d_M[ij] = min(s_colK[l_i] + s_filK[l_j], s_M[l_ij]);
    }
  }
}
...
//**************************************************************************************************
  ...
  int * c_out_M_2DSharedV2 = new int[nverts2];  // Matriz en el HOST 2D Shared V2
  ...
  int * d_In_M_2DSharedV2 = NULL;               // Matriz en DEVICE para 2D Shared V2

  int i, j, k;
  double T, Tgpu1D, Tgpu2D, Tgpu1DShared, Tgpu2DShared, Tgpu2DSharedV2, Tcpu;
  ...
  // Reservar espacio en memoria para la matriz en DEVICE
  err = cudaMalloc((void **) &d_In_M_2DSharedV2, size);
  if (err != cudaSuccess) {
    cout << "ERROR: Bad Allocation in Device Memory" << endl;
  }
  ...
  // Copiar los datos de la matriz en HOST en la matriz en DEVICE
  err = cudaMemcpy(d_In_M_2DSharedV2, G.Get_Matrix(), size, cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
    cout << "ERROR: COPY MATRIX TO DEVICE" << endl;
  }
  ...
  for (k = 0; k < niters; k++) {
    // Kernel Launch
    floyd_kernel2DSharedV2 <<< nblocks2D, blocksize2D >>> (d_In_M_2DSharedV2, nverts, k);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
      fprintf(stderr, "Failed to launch kernel!\n");
      exit(EXIT_FAILURE);
  ...
  // Copiar los datos de la matriz en DEVICE en la matriz en HOST
  cudaMemcpy(c_out_M_2DSharedV2, d_In_M_2DSharedV2, size, cudaMemcpyDeviceToHost);

  Tgpu2DSharedV2 = clock();
  ...
  cout << "Ganancia (2D Shared V2) = " << Tcpu / Tgpu2DSharedV2 << endl;
  ...
  for (i = 0; i < nverts; i++) {
    for (j = 0; j < nverts; j++) {
      ...
      if (abs(c_out_M_2DSharedV2[i * nverts + j] - G.arista(i, j)) > 0) {
        cout << "Error 2D Shared V2 (" << i << "," << j << ")   " << c_out_M_2DSharedV2[i * nverts + j]
             << "..." << G.arista(i, j) << endl;
      }
    }
  }
  ...
  cudaFree(d_In_M_2DSharedV2);
  delete[] c_out_M_1D;
  delete[] c_out_M_2D;
  delete[] c_out_M_1DShared;
  delete[] c_out_M_2DShared;
  delete[] c_out_M_2DSharedV2;
}
