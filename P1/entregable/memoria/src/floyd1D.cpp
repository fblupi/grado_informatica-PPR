...
int main (int argc, char *argv[]) 
{
  ...
  /**
    ...
    */
  MPI_Scatter(G.ptrMatriz(), tamaLocal, MPI_INT, &M[0][0], tamaLocal, MPI_INT, 0,
  		MPI_COMM_WORLD);
  ...
  for (k = 0; k < nverts; k++) {
    ...
    for (i = 0; i < tamaBloque; i++) { // Recorrer las filas (valores locales)
      ...
      for (j = 0; j < nverts; j++) {
        if (iGlobal != j && iGlobal != k && j != k) { // No iterar sobre diagonal
          vikj = M[i][k] + K[j];
          vikj = min(vikj, M[i][j]);
          M[i][j] = vikj;
        }
  ...
  /**
    ...
    */
  MPI_Gather(&M[0][0], tamaLocal, MPI_INT, G.ptrMatriz(), tamaLocal, MPI_INT, 0,
  		MPI_COMM_WORLD);
